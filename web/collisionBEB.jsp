...
<%
    ...
    String div="BEb";
     Class.forName("com.mysql.jdbc.Driver");
      ...
      ResultSet fac= st.executeQuery("select * from teach where subcode='"+subcode+"' and class_div='BEB'");
      String tag=null;
      while(fac.next())
      {
      ...
      if(cat.equals("class"))
      { 
        ResultSet insert= st.executeQuery("select * from timetable where ddid='"+ddid+"' and (divi='BEb' or room_no='"+room+"' or tag='"+tag+"')");
        while(insert.next())
      {
...
//int        
        ...
        if(d.equals("BEb") )
      {
            ...
            out.println("location='display_tableBEb.jsp'");
            out.println("</script>");
...
//int        
      ...
      if(r!=null && t!=null)
        ...
        else
        {
            ...
            ResultSet c=st.executeQuery("select count(*) from timetable where subcode='"+subcode+"' and divi='BEB'");
            while(c.next())
                cnt=Integer.parseInt(c.getObject(1).toString());
            if(cnt==limit || ddid%2==0)
            {
                out.println("<script type=\"text/javascript\">");
